...
BOOST_AUTO_TEST_CASE(Test_isIntervalX) {
	...
	BOOST_CHECK_EQUAL(CityController::isInIntervalX(Position(100, 100), street), false);
	BOOST_CHECK(CityController::isInIntervalX(Position(250, 250), street));
	BOOST_CHECK_EQUAL(CityController::isInIntervalX(Position(500, 500), street), false);
...
BOOST_AUTO_TEST_CASE(Test_isIntervalY) {
    ...
    PNode e = std::make_shared<Node>(Position(500, 500));
    PStreet street = std::make_shared<Street>(s, e);
	BOOST_CHECK_EQUAL(CityController::isInIntervalY(Position(100, 100), street), false);
	BOOST_CHECK(CityController::isInIntervalY(Position(250, 250), street));
	BOOST_CHECK_EQUAL(CityController::isInIntervalY(Position(500, 500), street), false);
...
BOOST_AUTO_TEST_CASE(Test_addSingleStreet) {
	...
	BOOST_CHECK_EQUAL(list.front()->getLength(), 400);
    BOOST_CHECK_EQUAL(list.front()->getDirection(), E);
...
BOOST_AUTO_TEST_CASE(Test_addMultiStreet) {
	...
	auto list = cityC->getStreets();
	BOOST_REQUIRE_EQUAL(list.size(), 8);
    BOOST_CHECK_EQUAL(cityC->getNodes().size(), 4);
    cityC->clearController();
}
...
BOOST_AUTO_TEST_CASE(Test_streetCrossing) {
	...
	BOOST_REQUIRE_EQUAL(cityC->getStreets().size(), 4);
    BOOST_CHECK_EQUAL(cityC->getNodes().size(), 5);
    cityC->clearController();
}
...
BOOST_AUTO_TEST_CASE(Test_city) {
	...
	auto list = cityC->getStreets();
	BOOST_REQUIRE_EQUAL(list.size(), 24);
    BOOST_CHECK_EQUAL(cityC->getNodes().size(), 13);
    cityC->clearController();
}
