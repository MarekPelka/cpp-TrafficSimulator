...
void Camera::updateObservation() {
    ...
    for (auto veh : vehicles) {
        ...
        if (checkIfInZone(angl)) {
            double distsqr = distanceBetweenPoints(position, veh.getPosition());
...
double Camera::getAngleBetweenPoints(Position p1, Position p2) {
    double angleRad = atan2(p1.y - p2.y, p1.x - p2.x);
...
double Camera::distanceBetweenPoints(Position p1, Position p2) {
